...
def correctECDS(folder_path, img_names, geonp_path, metadata_path, metadatanew_path, df, transformer, model):
    oldValues = [None, None]
    oldImgepath = ''
...
def correctELLK2(folder_path, img_names, geonp_path, metadata_path, metadatanew_path, df, transformer, model):
    oldValues = [None, None]
    oldImgepath = ''
    coordenadas_dict = df.set_index('name').to_dict(orient='index')
    for image_path in tqdm(img_names, desc="Calculando Offset E"):
...
def correctELine(folder_path, img_names, geonp_path, metadata_path, metadatanew_path, df, transformer, model, path_root, linesList):
    ...
    for e, line in enumerate(linesList):
        offsetList = []
        for image_path in tqdm(line, desc=f"Calculando Offset E {e}"):
